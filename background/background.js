...
function storeText(message) {
    const { side, text } = message;
    if (side === 'front') {
        cardState.frontText = text || null;
    } else if (side === 'back') {
        cardState.userBackText = text || null;
    }
    updateBadge();
    return { success: true };
}
...
function updateBadge() {
    const hasFront = cardState.frontImages.length > 0 || cardState.frontText;
    const hasBack = cardState.backImages.length > 0 || cardState.backText || cardState.userBackText;
...
async function addCard(message) {
    const { deckName, modelName, frontImages: msgFrontImages, frontText: msgFrontText, backImages: msgBackImages, backText: msgBackText } = message;

    const frontImgs = msgFrontImages || cardState.frontImages;
    const frontTxt = msgFrontText || cardState.frontText;
    const backImgs = msgBackImages || cardState.backImages;
    const backTxt = msgBackText || cardState.backText;

    if ((!frontImgs || frontImgs.length === 0) && !frontTxt) {
        return { success: false, error: '問題の内容がありません' };
    }
    ...
    try {
        ...
        cardState.frontImages = [];
        cardState.backImages = [];
        cardState.frontText = null;
        cardState.backText = null;
        cardState.userBackText = null;
